# Sky Observer 🔭

Independent astrophotography project — Bishkek, Kyrgyzstan 🇰🇬

**Founded:** March 2026  
**Equipment:** Celestron NexStar 127SLT (130mm aperture, 650mm focal length)  
**Location:** Bishkek, Kyrgyzstan (42.87°N, 74.59°E, elevation 800m)  
**TikTok:** @skyobserver08

---

## Session Log

### Session 1 — March 24, 2026 🌙
**Target:** First Quarter Moon  
**Equipment:** Celestron NexStar 1275LT + phone mount  
**Conditions:** Partly cloudy, ~19°C, Bishkek  
**Phase:** Waxing crescent, ~38% illuminated  
**Best result:** Terminator detail visible, individual crater rims resolved  
**Key learning:** Image sharpness improves significantly after telescope 
cools to ambient temperature.  
**Photos:** session-01 folder  

---

### Session 2 — ~March 26, 2026 🌙
**Targets:** Moon craters (close-up), Jupiter, Betelgeuse, Spica  
**Conditions:** Bishkek  
**Results:**  
- Moon: crater detail significantly improved at higher magnification  
- Jupiter: visible but image quality limited — stacking planned  
- Betelgeuse: captured, atmospheric distortion present  
- Spica: captured  
**Key learning:** Planetary photography requires stacking to overcome 
atmospheric turbulence. Stars show distortion at high magnification.  
**Photos:** session-02 folder  

---

### Session 3 — ~March 28, 2026 🌙
**Targets:** Moon, deep field stars  
**Conditions:** Bishkek  
**Results:** Continued lunar detail work at varying magnifications.  
Additional star targets captured.  
**Key learning:** Tracking consistency improves with each alignment.  
**Photos:** session-03 folder  

---

### Session 4 — ~March 30, 2026 🌙⭐
**Targets:** Moon, Betelgeuse, background stars  
**Conditions:** Bishkek  
**Results:** Multiple stellar objects captured. Building familiarity 
with GoTo system for rapid target acquisition.  
**Photos:** session-04 folder  

---

### Session 5 — ~April 1, 2026 🌕🪐
**Targets:** Full Moon (99.5% illumination), Jupiter  
**Conditions:** Bishkek  
**Phase:** Near-full Moon — 99.5% illuminated  
**Results:**  
- Moon: best capture to date — near-full illumination, 
  surface detail across entire disc  
- Jupiter: captured, image stacking planned for next session  
**Key learning:** Near-full Moon washes out nearby deep sky objects 
due to light pollution. Schedule galaxy/nebula sessions around 
new Moon phase for best results.  
**Photos:** session-05 folder  

---
---

## Equipment
| Item | Spec |
|------|------|
| Telescope | Celestron NexStar 127SLT |
| Aperture | 130mm |
| Focal length | 650mm |
| Mount | Computerized GoTo Alt-Az |
| Camera | iPhone + phone mount adapter |
